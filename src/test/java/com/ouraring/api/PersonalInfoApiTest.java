...
/**
 * API tests for PersonalInfoApi
 */
public class PersonalInfoApiTest extends ApiTest {

    private final PersonalInfoApi api = new PersonalInfoApi(apiClient);
     ...
     */
    @Test
    public void personalInfoRouteGetTest() throws Exception {
        CompletableFuture<PersonalInfoResponse> response =
                api.personalInfoRouteGet();

        PersonalInfoResponse piResponse = response.get(1L, TimeUnit.SECONDS);
        assertEquals(Integer.valueOf(25), piResponse.getAge());
        assertEquals(BigDecimal.valueOf(180), piResponse.getHeight());
        assertEquals(BigDecimal.valueOf(80), piResponse.getWeight());
        assertEquals("M", piResponse.getBiologicalSex());
        assertEquals("foo@bar.org", piResponse.getEmail());
    }
