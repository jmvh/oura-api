 ...
 */
public class WorkoutsApiTest extends ApiTest {

    private final WorkoutsApi api = new WorkoutsApi(apiClient);

    @Before
    public void setup() throws JsonProcessingException {
        mockServerClient.when(
                HttpRequest.request()
                        .withPath("/v2/usercollection/workout")
                ...
                HttpResponse.response()
                        ...
                        .withBody(
                                objectMapper.writeValueAsString(
                                        new WorkoutResponse().addDataItem(
                                                new WorkoutModel()
                                                        .day(LocalDate.now())
                                                        .activity("Serious grinding")
                                                        .calories(BigDecimal.valueOf(1500))
                                                        .distance(BigDecimal.valueOf(15000))
     ...
     */
    @Test
    public void workoutsRouteWorkoutGetTest() throws Exception {
        LocalDate startDate = null;
        LocalDate endDate = null;
        String nextToken = null;
        CompletableFuture<WorkoutResponse> response = 
        api.workoutsRouteWorkoutGet(startDate, endDate, nextToken);

        List<WorkoutModel> data = response.get(1L, TimeUnit.SECONDS).getData();
        assertEquals(1, data.size());
        assertEquals(LocalDate.now(), data.get(0).getDay());
        assertEquals("Serious grinding", data.get(0).getActivity());
        assertEquals(BigDecimal.valueOf(1500), data.get(0).getCalories());
        assertEquals(BigDecimal.valueOf(15000), data.get(0).getDistance());
        assertEquals("Super intense", data.get(0).getIntensity());
    }
