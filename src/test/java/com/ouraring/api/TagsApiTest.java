 ...
 */
public class TagsApiTest extends ApiTest {

    private final TagsApi api = new TagsApi(apiClient);

    @Before
    public void setup() throws JsonProcessingException {
        mockServerClient.when(
                HttpRequest.request()
                        .withPath("/v2/usercollection/tag")
     ...
     */
    @Test
    public void tagsRouteTagGetTest() throws Exception {
        LocalDate startDate = null;
        LocalDate endDate = null;
        String nextToken = null;
        CompletableFuture<TagResponse> response =
                api.tagsRouteTagGet(startDate, endDate, nextToken);

        List<TagModel> data = response.get(1L, TimeUnit.SECONDS).getData();
        assertEquals(1, data.size());
        assertEquals(LocalDate.now(), data.get(0).getDay());
        assertEquals("My tags", data.get(0).getText());
        assertEquals(2, data.get(0).getTags().size());
        assertTrue(data.get(0).getTags().containsAll(Arrays.asList("Workout", "Nap")));
    }
